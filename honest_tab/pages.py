...
from honest_tab.aux import two_decimal_points
from honest_tab.constants import *
...
def index() -> rx.Component:
  ...
  return rx.container(
    rx.center(
      rx.vstack(
        rx.hstack(
          rx.text("Welcome to", size='3'),
          justify="center", width="100%",
        ),
        rx.hstack(
          rx.heading(
            "HonestTab self-service",
        ...
        rx.dialog.content(
          ...
          rx.vstack(
            rx.form(
              ...
              rx.flex(
                rx.dialog.close(
                  rx.button("Register", type="submit", size=default_button_size)
                ), 
